...
import { useMap } from "./MapBase";

export interface AnnotationLayerBaseProps {
...
export const AnnotationLayerBase = memo(function AnnotationLayerBase({
  annotations,
  currentTime,
  selectedAnnotationId,
  onSelect,
}: AnnotationLayerBaseProps) {
  const { map } = useMap();
  ...
  }, [annotations, map, currentTime, selectedAnnotationId, onSelect, applyZoom]);

  useEffect(() => () => {
    for (const m of Array.from(markersRef.current.values())) m.remove();
